    ...
    virtual void ReadyToRun() {
        ...
        if (!entry.Exists()) {
            ...
            if (file.InitCheck() == B_OK) {
                const char* templateText = "#!/bin/bash\n# SleepWalker Template\n";
...
int main(int argc, char** argv) {
    ...
    if (argc > 1 && (strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0)) {
        ...
        printf("  SleepWalker -status  Check if the daemon is currently active\n");
    ...
    // 4. Check if already running before starting a new instance
    if (be_roster->IsRunning(sig)) {
    ...
    if (app.InitCheck() != B_OK) {
        fprintf(stderr, "Error: Could not initialize SleepWalker.\n");
        return 1;
    }
